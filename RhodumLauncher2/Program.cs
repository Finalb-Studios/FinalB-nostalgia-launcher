    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                string verget = wClient.DownloadString("http://cdn.finalb.xyz/launcher/version.txt");

                // check for HWID bans
                try
                {
                    string hwidbans = wClient.DownloadString("http://cdn.finalb.xyz/launcher/bans.txt");
                ...
                {
                    MessageBox.Show("You are using an outdated version of Finalb. Please download the new version.");
                    System.Environment.Exit(0);
                }
                try
                {
                    ...
                    string lansecurity = "http://cdn.finalb.xyz/launcher/securitycheck.php?key=" + jsoncode.key + "&uid=" + jsoncode.userid + "&gameId=" + jsoncode.gameid + "&invkey=" + hwid;
                    ...
                    {
                        // start the game, lol
                        string gamenameurl = "https://cdn.finalb.xyz/launcher/gamename.php?key=Iasdfaxwf03h50nbcinvklnhdhkh&uid=" + jsoncode.gameid;
                        string gamename = wClient.DownloadString(gamenameurl);
                        //discord.launchrpc(gamename);
                        string procPath = "";
                        procPath = Directory.GetCurrentDirectory() + "\\" + "Finalb.exe";
                        ProcessStartInfo RhodumClient = new ProcessStartInfo(procPath);
                        Process mainProcess;
                        RhodumClient.Arguments = "-script \"wait(); dofile('http://cdn.finalb.xyz/user/game/getScript2010.php?userID=" + jsoncode.userid + "&gameID=" + jsoncode.gameid + "&key=9cBOle3VIeU0wBfZmkL92qNU63xk8Y90&pkey=" + jsoncode.key + "')\"";
                        mainProcess = Process.Start(RhodumClient);
                        Console.Clear();
                        Console.ForegroundColor = ConsoleColor.Green;
                        Console.WriteLine("FinalB is now running! Do not close this window or the game will crash, this window will close automatically when you stop playing.");
